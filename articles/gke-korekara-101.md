...
Node の手動アップグレードは [Node Pool](https://cloud.google.com/kubernetes-engine/docs/concepts/node-pools) (同一マシンサイズ・構成の Node の集まり) 単位で以下 2 種類のアップグレード方式から選択可能です。(Release Channel など自動アップグレード利用の場合はサージアップグレードが選択されます)
